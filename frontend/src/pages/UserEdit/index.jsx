...
export function UserEdit() {
    ...
    const updatePassword = async () => {
        ...
        if(password === passwordConfirmation) {
            ...
            try {
                ...
                setUpdated(prev =>[...prev,{ password: true, passwordSubmit: true}])
    ...
    const nameChange = ({target}) => {
        target.value !== user.name && setUpdated(prev => [{...prev, basicsSubmit:true}])
        setName(target.value)
    }

    const emailChange = ({target}) => {
        target.value !== user.email && setUpdated(prev => [{...prev, basicsSubmit:true}])
            ...
            <div className='userEdit__organizedSet'>
                ...
                <SimpleBox>
                    ...
                    <div className='userEdit__submit'>
                        <button onClick={updateBasics} className={!updated[0].basicsSubmit?'disabled':''}>
                            Update
                        </button>
                    </div>
                    
                    {
                        isOpen[0].basics && 
                        <div 
                        className={errors[0].basics ? 'userEdit__error' : 'userEdit__success'}
                        data-active={!updated[0].basics | !errors[0].basics?true:false}>
                            {errors[0].basics ? 'Update failed, try again' : 'Updated successfully!'}
                        </div>
                    }
            ...
            <div className='userEdit__organizedSet'>
                ...
                <SimpleBox>
                    ...
                    <div className='userEdit__submit'>
                        ...
                        className={password | passwordConfirmation === '' ?'disabled':''}>
                            Update Password
                        </button>
                    </div>
                    ...
                    
                    {
                        ...
                        className={
                            ...
                            ? 'userEdit__error' : 'userEdit__success'
                        }
                        data-active={!updated[0].password?true:false}>
                            {errors[0].samePassword && 'Passwords are not the same'}
                            {errors[0].password & !errors[0].samePassword ? 'Update failed, try again': ''}
                            {!errors[0].password & !errors[0].samePassword ? 'Updated successfully!': ''}
                        </div>
                    }
                </SimpleBox>
            </div>
